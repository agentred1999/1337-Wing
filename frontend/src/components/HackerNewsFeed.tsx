...
type SortOption = 'rel' | 'pts' | 'new' | 'cmt'
type TypeFilter = 'all' | 'story' | 'ask_hn' | 'show_hn' | 'job'

export default function HackerNewsFeed() {
  const [query, setQuery] = useState('security hacking linux pentest')
  const [inputVal, setInputVal] = useState('')
  const [sort, setSort] = useState<SortOption>('rel')
  const [type, setType] = useState<TypeFilter>('all')
  const [minPts, setMinPts] = useState(0)
  const [rawHits, setRawHits] = useState<HNHit[]>([])
    ...
    <section id="hn-section" aria-labelledby="hn-heading">
      ...
      <div className="hn-inner">
        ...
        <div className="hn-filters">
          ...
          <select id="hn-sort" value={sort} onChange={e => setSort(e.target.value as SortOption)}>
          ...
          <select id="hn-type" value={type} onChange={e => setType(e.target.value as TypeFilter)}>
